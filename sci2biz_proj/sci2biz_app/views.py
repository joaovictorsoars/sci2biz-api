...
from sci2biz_app.models import Users, Roles, Turma, UsuarioTurma
        ...
        try:
            ...
            if refresh_input and refresh_input.startswith("Bearer "):
                ...
                if user.refresh_token != str(refresh):
                    return JsonResponse(
                        {"message": "Token de atualização inválido"}, status=400
                    )
                ...
                return JsonResponse(
                    {"message": "Token de atualização é necessário"}, status=400
                )
    ...
    if request.method == "POST":
        try:
            ...
            if user.is_active:
                ...
                return JsonResponse(
                    {"message": "Conta de usuário desativada"}, status=403
                )
    ...
    if request.method == "POST":
        try:
            ...
            if Users.objects.filter(email=email).exists():
                return JsonResponse(
                    {"message": "Este email já está registrado"}, status=400
                )
    ...
    if request.method == "PUT":
        ...
        if boolean:
            try:
                ...
                user.save()
                return JsonResponse(
                    {"message": "Usuário atualizado com sucesso"}, status=200
                )
    ...
    if request.method == "PUT":
        ...
        if boolean:
            try:
                ...
                return JsonResponse(
                    {"message": "Status de atividade do usuário alterado com sucesso"},
                    status=200,
                )
...
def request_password_reset(request) -> JsonResponse:
    ...
    if request.method == "POST":
        try:
            ...
            return JsonResponse(
                {"message": "Email de redefinição de senha enviado"}, status=200
            )
...
def confirm_password_reset(request, uidb64, token) -> JsonResponse:
    ...
    if request.method == "POST":
        try:
            ...
            if not new_password:
                return JsonResponse(
                    {"message": "A nova senha é obrigatória"}, status=400
                )
        ...
        if auth_header and auth_header.startswith("Bearer "):
            ...
            try:
                ...
                if user.role_id.role_name != "Professor":
                    return JsonResponse(
                        {
                            "error": "Acesso negado. Apenas Professores podem criar demandas."
                        },
                        status=403,
                    )
                ...
                if not set(tipo_demanda).issubset(tipos_validos):
                    return JsonResponse(
                        {"message": "Tipo de demanda inválido"}, status=400
                    )
                ...
                admin_emails = Users.objects.filter(
                ...
                send_mail(
                    email_subject, email_body, settings.DEFAULT_FROM_EMAIL, admin_emails
                )

                return JsonResponse(
                    {"message": "Demanda criada com sucesso"}, status=201
                )
        ...
        if auth_header and auth_header.startswith("Bearer "):
            ...
            try:
                ...
                if user.role_id.role_name != "Admin":
                    return JsonResponse(
                        {
                            "error": "Acesso negado. Apenas Admins podem acessar demandas."
                        },
                        status=403,
                    )
                        ...
                        **(
                            {
                                ...
                                "data_resposta": demanda.data_resposta,
                            }
                            if demanda.data_resposta
                            else {}
                        ),
                    }
        ...
        if auth_header and auth_header.startswith("Bearer "):
            ...
            try:
                ...
                if user.role_id.role_name != "Admin":
                    return JsonResponse(
                        {
                            "error": "Acesso negado. Apenas Admins podem responder a demandas."
                        },
                        status=403,
                    )
                ...
                return JsonResponse(
                    {"message": "Resposta à demanda atualizada com sucesso"}, status=200
                )
            except (InvalidToken, TokenError):
                return JsonResponse({"error": "Token inválido"}, status=401)
            except Users.DoesNotExist:
...
def update_demanda(request, demanda_id) -> JsonResponse:
    ...
    if request.method == "PUT":
        try:
            ...
            demanda.indicacao_ativa = data.get(
                "indicacao_ativa", demanda.indicacao_ativa
            )
            demanda.fluxo = data.get("fluxo", demanda.fluxo)
            demanda.perspectiva = data.get("perspectiva", demanda.perspectiva)
            demanda.orientacoes_pesquisa = data.get(
                "orientacoes_pesquisa", demanda.orientacoes_pesquisa
            )
            ...
            demanda.save()
            return JsonResponse(
                {"message": "Demanda atualizada com sucesso"}, status=200
            )
        ...
        if auth_header and auth_header.startswith("Bearer "):
            ...
            try:
                ...
                if user.role_id.role_name != "Professor":
                    return JsonResponse(
                        {
                            "error": "Acesso negado. Apenas Professores podem criar turmas."
                        },
                        status=403,
                    )
...
@csrf_protect
def add_monitor(request) -> JsonResponse:
    """Add a monitor to a Turma."""
    if request.method == "POST":
        auth_header = request.headers.get("Authorization")
        if auth_header and auth_header.startswith("Bearer "):
            token = auth_header.split(" ")[1]
            try:
                ...
                if user.role_id.role_name != "Professor":
                    return JsonResponse(
                        {
                            "error": "Acesso negado. Apenas Professores podem adicionar monitores."
                        },
                        status=403,
                    )
                ...
                monitor_email = data.get("monitor_email")

                if not turma_id or not monitor_email:
                    return JsonResponse(
                        {"message": "Campos obrigatórios faltando"}, status=400
                    )

                turma = Turma.objects.get(id=turma_id)
                monitor = Users.objects.get(email=monitor_email)

                if monitor.role_id.role_name != "Aluno":
                    return JsonResponse(
                        {"message": "Apenas Alunos podem ser monitores"}, status=400
                    )

                monitor.role_id = Roles.objects.get(role_name="Monitor")
                monitor.save(update_fields=["role_id"])

                return JsonResponse(
                    {"message": "Monitor adicionado com sucesso"}, status=200
                )
...
@csrf_protect
def add_student(request) -> JsonResponse:
    """Add a student to a Turma."""
    if request.method == "POST":
        auth_header = request.headers.get("Authorization")
        if auth_header and auth_header.startswith("Bearer "):
            token = auth_header.split(" ")[1]
            try:
                ...
                if user.role_id.role_name != "Professor":
                    return JsonResponse(
                        {
                            "error": "Acesso negado. Apenas Professores podem adicionar alunos."
                        },
                        status=403,
                    )
                ...
                if not turma_id or not student_name or not student_email:
                    return JsonResponse(
                        {"message": "Campos obrigatórios faltando"}, status=400
                    )

                turma = Turma.objects.get(id=turma_id)
                student, created = Users.objects.get_or_create(
                    email=student_email,
                    defaults={
                        "full_name": student_name,
                        "role_id": Roles.objects.get(role_name="Aluno"),
                    },
                )
                ...
                email_body = f"Olá, {student_name}!\n\nVocê foi adicionado à turma {turma.nome}. Por favor, complete seu cadastro no sistema."
                send_mail(
                    email_subject,
                    email_body,
                    settings.DEFAULT_FROM_EMAIL,
                ...
                return JsonResponse(
                    {"message": "Aluno adicionado com sucesso"}, status=200
                )
...
@csrf_protect
def import_students(request) -> JsonResponse:
            ...
            if auth_header and auth_header.startswith("Bearer "):
                ...
                try:
                    ...
                    if user.role_id.role_name != "Professor":
                        return JsonResponse(
                            {
                                "error": "Acesso negado. Apenas Professores podem importar alunos."
                            },
                            status=403,
                        )
                    ...
                    for _, row in df.iterrows():
                        ...
                        UsuarioTurma.objects.get_or_create(
                            turma_id=turma, usuario_id=student
                        )
                    ...
                    return JsonResponse(
                        {"message": "Alunos importados com sucesso"}, status=200
                    )
                    ...
                    return JsonResponse(
                        {"message": "Função não encontrada"}, status=404
                    )
                except (KeyError, JSONDecodeError):
                    return JsonResponse({"message": "JSON inválido"}, status=400)
                except Exception as e:
